...
-- 단일 행 함수 : N 개의 값을 읽어서 연산 후 N개의 결과를 반환

-- 그룹 함수 : N개의 값을 읽어서 연산 후 1개의 결과를 반화(합계, 평균, 최대, 최소)
...
-- 저정한 위치 부터 지정한 순번째로 검색되는 문자의 위치를 반환

-- AABAACAABBAA

-- 문자열 앞에서부터 검색하여 첫번째 B 위치 조회
...
-- 주어진 컬럼이나 문자열으이 앞, 위, 양쪽에 있는 지정된 문자를 제거
--> 양쪽 공백 제거에 많이 사용함
...
-- ROUND(숫자 | 컬럼명 [,소수점 위치]) : 반올임

SELECT ROUND (123.456) FROM DUAL; --123, 소수점 첫번째 자리에서 반올림

SELECT ROUND(123.456, 1) FROM DAUL; 
...
SELECT SYSDATE FROM DUAL; -- 2025-03-10 12:11:09.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:12:21.398 +0900
...
-- TO_DATE(문자형데이터, [포멧]) : 문자형 -> 날짜로 변경
-- TO_DATE(숫자형데이터, [포멧]) : 숫자형 -> 날짜로 변경
--> 지정된 포멧으로 날짜를 인식함
...
-- 3) TO_CHAR를 이용해서 문자열로 변환 -> 1965년 12월 31일
SELECT EMP_NAME, 
...
-- NULL 처리 함수
-- NVL(컬럼명, 컬럼값이 NULL 일 때 바꿀 값) : NULL 인 컬럼값을 다른 값으로 변경
SELECT * FROM EMPLOYEE;
...
-- 급여 최대 값, 가장 최신 입사일, 알파벳 순서가 가장 느린 이메일 조회

...
-- 왜 사용할까?
--> VIEW를 이용새서  DML(INSERT/UPDATE/DELETE) 막기 위해서!
...
-- 200까지 증가 시켜서 변경 확인
SELECT SEQ_TEST_NO.NEXTVAL FROM DAUL;
...
BEGIN
	FOR I IN 1..1000000 -- I 는  100만까지 반복
	LOOP 
		INSERT INTO TB_IDX_TEST
		VALUES(I, 'TEST' || I); -- TEST 1 ~ TEST 100만
	END LOOP;
...
/* 인덱스 X */
-- TEST_ID 가 'TEST5000000' 인 행 조회 하기
SELECT * FROM TBN_IDX_TEST
WHERE TEST_ID = 'TEST500000'; -- 

/* 인덱스 O */
-- TEST_NO 가 5000000인 행을 조회하기
SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 5000000; -- 0.001S

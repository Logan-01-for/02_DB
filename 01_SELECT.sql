...
이 때, 조회된 행들의 집합을 "RUSULT SET"이라고 한다.

-RESULT SET 은 0개 이상의 행을 포함 할 수있다.
...
-- 주의 사항 2) DISTINCT 구문은 SELECT 제일 앞에 작성 되어야 한다.
...
-- 4. ORDER BY 절(정렬) : ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC]
-- [NULLS FIRT | LAST]

-- EMPLOYEE 테이블에서 (FROM 절)
-- 급여가 3백만원 초과인 사원의 (WHERE 절)
-- 사변, 이름, 급여, 부서코드를 조회해라. (SELECT 절)
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE OB_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; 
...
-- EMPLOYEE 테이블에서 EMAIL의 _ 앞에 글자가 3글자인 사원만 
-- 이름, 이메일 조회
SELECT * FROM EMPLOYEE;
-- sun_di@or.kr
-- song_jk@or.kr

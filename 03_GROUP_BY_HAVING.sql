...
-- 여러개의 결과 값을 산출하기 위해 그룹함수가 적용된 그룹의 기준을 ORDER BY 절에 기술하여 사용
...
FROM EMPLOYEE
GROUP BY DEPT_CODE;
-- DEPT_CODE 컬럼을 그룹으로 묶어, 그 그룹의급여 합계 (SUM(SALARY))를 구함
...
SUM(SALARY) "급여 합"
/*1*/FROM EMPLOYEE 
/*2*/GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2','여') -- 별칭 사용 X(SELECT 절 해석 X)
/*4*/ORDER BY "인원 수"; -- 별칭사용 O (SELECT 절 해석 완료)


---------------------------------------------------------
-- * WHERE 절 GROUP BUY 절 혼합하여 사용하기 *

--> WHERE 절은 각 컬럼값에 대한 조건
--> HAVING 절은 그룹에 대한 조건

-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서 코드가 D5, D6인 부서의 (WHERE 절  -> GROUP BY 절)
-- 부서 코드, 평균 급여, 인원 수 조회(SELECT 절)
SELECT DEPT_CODE, ROUND(AVG(SALARY)), COUNT(*)
FROM EMPLOYEE 
WHERE DEPT_CODE IN('D5', 'D6')-- 부서 코드가 D5, D6로 거름
GROUP BY DEPT_CODE; -- WHERE 절에서 구함 부서코드가 D5, D6인 데이들을 그룹으로 묶음
...
-- SUBSTR(TO_CHAR(HIRE_DATE, 'YYY'), 1, 4) >= '2000'
GROUP BY JOB_CODE;
...
-- 그룹함수가 적용되지 않은 컬럼을 
-- 모두 GROUP BY 절에 작성 되어있어야 함.

-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서별로 같은 직급인 사원의 인원수를 조회 (GROUP BY 절 DEPT_CODE, JOB_CODE 묶음)
-- 부서 코드 오름차순, 직급코드 내림차순으로 정렬(ORDER BY 절)
-- 부서코드, 직급코드, 인원수(SELECT 절)

SELECT DEPT_CODE, JOB_CODE, COUNT(*)-- COUNT(*) : 인원수
FROM EMPLOYEE
...
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 평균이 3백만원 이상인 그룹만 조회
ORDER BY DEPT_CODE;
 ...
 * 
 * - MINOU (차집합) : A에서 A,B 교집합 부분을 제거하고 조회
 * 
 * */
...
WHERE DEPT_CODE = 'D5'
MINUS 
-- (2번째 SELECT문) 급여가 300 만원 초과인 사원의 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3000000;

-- (주의 사항!) 집합연산자를 사용하기 위한 SELECT문들은

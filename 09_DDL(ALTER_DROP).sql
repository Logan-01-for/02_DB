...
-- DEPY_COPY의 DEPT_TITLE 컬럼에 UNIQUE 추가
ALTER TABLE DEPT_COPY 
ADD CONSTRAINT DEPT_COPY_TITLE_U UNIQUE(DEPT_TITLE); -- 제약조건 명
...
SELECT * FROM DEPT_COPY;
--> 컬럼이 생성 되면서 DEFUALT 값이 자동 삽입되어있음

-- D10 개발 1팀 추가
INSERT INTO DEPT_COPY 
VALUES('D10', '개발 1팀', 'L1', NULL, DEFAULT);
-- ORA-12899: "KH"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID 의 데이터 타임이 CHAR(2) 이므로 영어+숫자 2글자까지 저장 가능
...
SELECT * FROM DEPT_COPY;
--> !! 기본 값ㅇ르 변경했다고 해서 기존 데이터가 변하지는 않음!
...
-- DEPARTMENT 테이블 복사해서 DEPT_ COPY 생성
CREATE TABLE DEPT_COPY 
AS SELECT * FROM DEPARTMENT;
--> 컬럼명, 데이터 타임, NOT NULL 여부만 복사

-- DEPT_COPY 테이블 PRIMARY KEY 추가 
-- (컬럼 : DEPT_ID, 제약 조건명 : D_COP_PK)
ALTER TABLE DEPT_COPY ADD CONSTRAINT D_COP_PK
PRIMARY KEY (DEPT_ID);
...
TB2_COL NUMBER REFERENCES TB1 -- FK 제약 조건 설정
); -- 자식 테이블
...
-- 3) DROP TBALE 삭제옵션 CASCAD CONSTRAINTS 사용
--> CASCAD CONSTRAINTS : 삭제하려는 테이블과 연결된 FK 제약조건을 모두 삭제
...
/*DDL 주의 사항*/
-- 1) DDL은 COMMMIT/ROLLBACK 이 되지 않는다.
...
-- > DDL이 종료된 후 DML구문을 수행 할 수 있도록 권장!
...
SELECT * FROM TB2;
-- 롤백이 안된다... 위에서 DDL구문 중 ALTER를 사용해서

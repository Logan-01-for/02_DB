 ...
 * 		EXECUTE			 		PROCEDURE (실행 권한 - 프로시져(함수)를 실행할 권한)
 * 
 * */
...
-- CREATE TABLE : 테이블 생성 권한
-- + 데이터를 저장 할 수 있는 공간 (TABLESPACE) 할당

-- 6. (sys) 테이블 생성 권한 부여 + TABLESPACE 할당
GRANT CREATE TABLE TO cmh_sample;

ALTER USER cmh_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

-- 7. (sample) 테이블 다시 생성
CREATE TABLE TB_TEST(
PKI_COL NUMBER PRIMARY KEY,
CONTENT VARCHAR2 
);
-------------------------------------------------------------
...
-- CONNECT / RESOURCE : 사용자에게 부여할 수있는 기본적인 시스템 역할.

-- CONNECT : DB 접속 관련 권한을 묶어둔 ROLE
--> CREATE SESION, ALTER SESSION..

...
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
-- WHERE DEPT_TITLE IN('해외영업 1부', '해외영업 2부')
...
SELECT EMP_NAME 사원명, DEPT_TITLE 지역명, LOCATION_ID 지역명, NATIONAL_CODE 국가명
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_TITLE)
JOIN LOCATION ON (LOCAL_NAME = LOCATION_ID)
...
-- 조회하시오.(SELF JOIN 사용)
SELECT * FROM EMPLOYEE;
SELECT E.EMP_NAME AS 사원이름, E.DEPT_CODE AS 부서코드, P.EMP_NAME AS 동료이름
FROM EMPLOYEE E

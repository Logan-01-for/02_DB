...
-- 사번, 이름, 부서코드, EMPLOYEE 테이블에서 조회 가능
...
-- EMPLOYEE 테이블의 DEPT-CODE 와
-- DEPARTMENT 테이블의 DEPT_ID를 연결 고리 지정
...
-- 1) LEFT [OUTER] JOIN : 합치기에 사용한 두 테이블 중 왼편에 기술되 테이블의
-- 컬럼수를 기준으로 JOIN
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) -- (EMPLLOYEE + JOB) + DEPARTMENT
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)-- (EMPLLOYEE + JOB + DEPARTMENT) + LOCATION
WHERE JOB_NAME ='대리' 
...
SELECT E.EMP_NAME, E.EMP_NO, D.DEPT_TITLE, E.JOB_CODE, 
FROM EMPLOYEE
JOIN DEPARTMENT ON (E.DEPT_ID = D.DEPT_ID)
JOIN DEPARTMENT ON(E.JOB_ID = D.JOB_ID)
WHERE EMP_NAME = '전%', SUBSTR(EMP_NO, 7%, 2%);
...
WHERE D.DEPT_TITLE IN ('해외영업 1부', '해외영업 2부');
...
SELECT EMP_NAME, JOB_CODE, DEPT_ID, LOCATION_ID
FROM EMPLOYEE, DEPARTMENT, LOCATION


-- 6. 급여등급별 최소급여(MIN_SAL)를 초과해서 받는 직원들의 사원명, 직급명,
-- 급여, 연봉(보너스포함)을 조회하시오. (연봉에 보너스포인트를 적용하시오.)
SELECT DEPT_CODE, JOB_CODE, SALARY, BONUS
FROM EMPLOYEE;

-- 7.한국(KO)과 일본(JP)에 근무하는 직원들의 사원명, 부서명, 지역명, 국가명을
-- 조회하시오.
SELECT EMP_NAME, JOB_CODE, DEPT_ID, LOCATION_ID, NATONAL_CODE
FROM EMPLOYEE, DEPARTMENT, LOCATION 
WHERE "NATIONAL"; 
...
-- 조회하시오.(SELF JOIN 사용)
SELECT * FROM EMPLOYEE;
SELECT EMP_ID 사번, EMP_NAME 사원이름, DEPT_CODE 부서코드 
NVL(EMP_NAME) "이름"
FROM EMPLOYEE;

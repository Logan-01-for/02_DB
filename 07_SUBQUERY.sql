 ...
 * =, != 
 * - IN / NOT IN : 여러개의 결과 값중에서 한개라도 일치하는 값이 있다면
 * 								혹은 없다면 이라는 의미(가장 많이 사용!)
 * 
 * - > ANY, < ANY : 여러개의 결과 값 중에서 한 개라도 큰 / 작은 경우
 * 							--> 가장 작은 값보다 큰가? / 가장 큰 값보다 작은가?
 * 
 * - > ALL, < ALL : 여러개의 결과 값의 모든 값보다 큰 / 작은 경우
...
-- 1) 직급이 대리인 직원드르이 사번, 이름, 직급, 급여 조회(메인쿼리)
...
-- 상관쿼리는 먼저 메인 쿼리 한 행을 조회 하고
-- 해당 행이 서브쿼리의 조건을 충족하는지 확인하여 SELECT 를 진행함

-- ** 해석 순서가 기존 서브쿼리와는 다르다 **
-- 메인 쿼리 1행 -> 1행에 대한 서브쿼리를 수행
-- 메인 쿼리 2행 -> 2행에 대한 서브쿼리를 수행..
-- ...
-- 메인쿼리의 행의 수 만큼 서브쿼리사 생성되어 진행됨
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D2'; -- 2007-03-20 00:00:00.000

-- 상관쿼리 (메인 쿼리에서 이태림 포함/ 서브쿼리에서 거름)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음')
JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
...
-- SQL에서 단일값 '스칼라' 라고 함.
-- 즉, SELECT절에 작성되는 단일행 단일열 서브쿼리를 스칼라 서브 쿼리라고 함

-- 모든 직원의 이름, 직급, 급여, 전체 사원 중 가장 높은 급여를 받는 사람
깃
SELECT EMP_NAME, JOB_CODE, SALARY,
(SELECT MAX (SALARY) FRO EMPLOTEE) - SALARY "급여"
FROM EMPLOYEE;
...
-- 1) 이름, 급여를 급여 내림차순으로 조ㅚ한 결과를 인라인뷰 사용
--> FROM 저렝 작성되기 때문에 해석 1순위
...
FROM (SELECT EMP_NAME, SALARY
			FROM EMPLOYEE
			ORDER BY SALARY DESC) -- 해석 순위 1위 FROM 절에서 전체 직원의 SALARY 내림 차순 정렬
WHERE ROWNUM <= 5; -- 해석 순위 2위 WHERE 절에서 가상 컬럼의 1~5행 까지만 조회


-- 급여 평균이 3위 안에 드는 부서의
-- 부서코드, 부서명, 평균급여 조회
-- 인라인뷰 (+GROUP BY)
SELECT DEPT_CODE, DEPT_TITLE, 평균_급여
FROM (
    SELECT DEPT_CODE, DEPT_TITLE, CEIL(AVG(SALARY)) AS 평균_급여
    FROM EMPLOYEE
    LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
